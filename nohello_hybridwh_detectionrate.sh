...
# ===== output dir =====
ROOT_DIR="results/hybrid/nohello"
...
for WH in "${WH_SIZES[@]}"; do
  ...
  for ((i=1; i<=RUN_COUNT; i++)); do
    ...
    RUN_STR="${SCENARIO} --size=${NODES} --time=${TIME} --WH_size=${WH} --end_distance=${END_DISTANCE} --iteration=${i} --result_file=${OUT} --forwardmode=1 --destination_only=${DEST_ONLY}"
